...
pub async fn cancel_provider_session(
    ...
    tracing::info!("Cancelling provider session for session: {:?}", session_id);
        ...
        match registry.0.get_provider_session_by_id(sid) {
            Ok(Some(process_info)) => {
                ...
                match registry.0.kill_process(process_info.run_id).await {
                    Ok(success) => {
                        if success {
                            tracing::info!("Successfully killed process via registry");
                            killed = true;
                        } else {
                            tracing::warn!("Registry kill returned false");
                        }
                    }
                    Err(e) => {
                        tracing::warn!("Failed to kill via registry: {}", e);
                    }
                }
                attempted_methods.push("registry");
            }
            Ok(None) => {
                tracing::warn!("Session {} not found in ProcessRegistry", sid);
            }
            Err(e) => {
                tracing::error!("Error querying ProcessRegistry: {}", e);
            }
        }
    }
    ...
    if !killed {
        ...
        if let Some(mut child) = current_process.take() {
            ...
            match child.kill().await {
                Ok(_) => {
                    tracing::info!("Successfully killed provider session via state");
                    killed = true;
                }
                Err(e) => {
                    tracing::error!("Failed to kill provider session via state: {}", e);

                    // Method 3: If we have a PID, try system kill as last resort
                    if let Some(pid) = pid {
                        tracing::info!("Attempting system kill as last resort for PID: {}", pid);
                        ...
                        match kill_result {
                            Ok(output) if output.status.success() => {
                                tracing::info!("Successfully killed process via system command");
                                killed = true;
                            }
                            Ok(output) => {
                            ...
                            Err(e) => {
                                tracing::error!("Failed to execute system kill command: {}", e);
                            }
                        }
                    }
                }
            ...
            tracing::warn!("No active provider session process in state");
        }
    }

    if !killed && attempted_methods.is_empty() {
        tracing::warn!("No active provider session process found to cancel");
    }
    ...
    if killed {
        tracing::info!("Provider session cancellation completed successfully");
...
async fn spawn_provider_session_process(
    ...
    tracing::info!("Spawned provider session process with PID: {:?}", pid);
    ...
    let provider_session_state = app.state::<ProviderSessionProcessState>();
    {
        ...
        if let Some(mut existing_child) = current_process.take() {
            tracing::warn!("Killing existing provider session process before starting new one");
    ...
    let stdout_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            tracing::info!("Provider session stdout: {}", &line[..line.len().min(200)]);
                    ...
                    if let Some(provider_session_id) = msg["session_id"].as_str() {
                        ...
                        if session_id_guard.is_none() {
                            *session_id_guard = Some(provider_session_id.to_string());
                            tracing::info!("Extracted provider session ID: {}", provider_session_id);
                            ...
                            match registry_clone.register_provider_session(
                                ...
                                Err(e) => {
                                    tracing::error!("Failed to register provider session: {}", e);
                                }
                            }
                        }
                    }
    ...
    let stderr_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            tracing::error!("Provider session stderr: {}", line);
    ...
    tokio::spawn(async move {
        ...
        if let Some(mut child) = current_process.take() {
            match child.wait().await {
                Ok(status) => {
                    tracing::info!("Provider session process exited with status: {}", status);
                ...
                Err(e) => {
                    tracing::error!("Failed to wait for provider session process: {}", e);
